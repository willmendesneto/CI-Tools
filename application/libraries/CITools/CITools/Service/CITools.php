...
use CITools\Config\CodeigniterConfig;
...
class CITools
{
    ...
    /**
     * Generate an encription_key for application
     *
     * php generator:key
     * @return type
     */
    public function key(){
        ...
        $encriptionKey = CodeigniterConfig::generatorEncriptionKey();
        $configFileDirectory = __DIR__ . self::$appDir . 'config' . DIRECTORY_SEPARATOR;

        if( !file_exists($configFileDirectory . 'config.php')){
            throw new \Exception("File '" . $configFileDirectory . "config.php' not found!");
        }

        $configFile = file_get_contents($configFileDirectory . 'config.php');

        if(strpos( '$config[\'encryption_key\'] = \'\';', $configFile) !== FALSE){
            throw new \Exception("Error: An application key already exists in '" . $configFileDirectory . "config.php'!");
        }

        $configFile = str_replace( '$config[\'encryption_key\'] = \'\';', '$config[\'encryption_key\'] = \''.$encriptionKey.'\';', $configFile);

        if ( file_put_contents($configFileDirectory . 'config.php', $configFile) !== FALSE ) {
            echo "[x] Configuration updated with secure key!";
            return;
        }
    }
...
';
    ...
    public function view($paths)
    {
        ...
        foreach( $paths as $path ) {
            ...
            $contentTitle = "This is the view file generated by CITools.";

            self::$content = CodeigniterTemplate::generatorView($path, $contentTitle);
